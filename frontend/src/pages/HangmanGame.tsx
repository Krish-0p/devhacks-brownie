...
import LetterBoard from '../components/hangman/LetterBoard';
import WordReveal from '../components/hangman/WordReveal';
import type { LeaderboardEntry } from '../lib/types';
import { mapPlayers } from '../hooks/useSocket';

const MAX_TIME = 90; // matches HANGMAN_CONFIG.roundTime
...
export default function HangmanGame() {
  ...
  const amDrawing = useGameStore(s => s.amDrawing); // "amDrawing" = word setter in hangman
  ...
  const maxWrongGuesses = useGameStore(s => s.maxWrongGuesses);
  ...
  useSocketEvent('round_start', useCallback((data) => {
    ...
    addMessage({
      id: createMsgId(),
      text: `${drawer} is setting the word!`,
      type: 'system',
      timestamp: Date.now(),
    });
  }, [setRound, addMessage]));
  ...
  useSocketEvent('you_are_drawing', useCallback((data) => {
    ...
    addMessage({
      id: createMsgId(),
      text: `You're the word setter! Word: ${word}`,
      type: 'hint',
      timestamp: Date.now(),
    });
  ...
  useSocketEvent('hangman_update', useCallback((data) => {
    const revealed = data.revealedWord as string[];
    const wrong = data.wrongGuesses as number;
    const letters = data.guessedLetters as string[];
    const max = data.maxWrongGuesses as number;
    setHangmanUpdate(revealed, wrong, letters, max);
  }, [setHangmanUpdate]));

  useSocketEvent('letter_result', useCallback((data) => {
    ...
    addMessage({
      id: createMsgId(),
      text: correct
        ? `${player} guessed "${letter}" — correct! ✓`
        : `${player} guessed "${letter}" — wrong! ✗`,
      type: correct ? 'correct' : 'system',
      timestamp: Date.now(),
    });
  }, [addMessage]));
  ...
  useSocketEvent('correct_guess', useCallback((data) => {
    ...
    addMessage({
      id: createMsgId(),
      text: `${player} guessed the full word! (+${data.score})`,
      type: 'correct',
      timestamp: Date.now(),
    });
  }, [addMessage]));
  ...
  const timerBg = timeLeft > 30 ? 'bg-green-400' : timeLeft > 15 ? 'bg-yellow-400' : 'bg-red-400';
    ...
    <div className="h-screen flex flex-col p-2 md:p-4 gap-3 overflow-hidden">
      ...
      <header className="flex items-center justify-between gap-3 z-20">
        ...
        <div className="flex items-center gap-3 shrink-0">
          <div className="bg-orange-800/60 backdrop-blur-md px-4 py-2 rounded-2xl border-2 border-orange-600/50 flex items-center gap-2 shadow-lg">
          ...
          {totalTurns > 0 && (
            <div className="bg-orange-800/60 backdrop-blur-md px-3 py-2 rounded-2xl border-2 border-orange-600/50 text-orange-200 text-xs font-bold shadow-lg hidden sm:block">
              Turn {currentTurn}/{totalTurns}
            </div>
          )}
        </div>

        {/* Center: word display / hangman status */}
        <div className="flex-1 flex justify-center">
          <div className="bg-orange-400 border-4 border-black rounded-2xl px-6 py-2 shadow-[4px_4px_0_0_rgba(0,0,0,1)] max-w-sm w-full text-center transform -rotate-1">
            <p className="font-display font-black text-black text-lg md:text-2xl tracking-[0.15em] leading-tight">
              {amDrawing
                ? `Your word: ${wordDisplay}`
                : wordDisplay
                  ? `${wordDisplay.replace(/_/g, '_ ')}`
                  : 'HANGMAN'
              }
            </p>
          </div>
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <div className="hidden lg:block w-32 bg-orange-900/40 rounded-full h-3 border border-orange-700/50 overflow-hidden">
      ...
      {/* ── Main Layout: Sidebar | Hangman Area | Chat ── */}
      <div className="flex-1 flex flex-col md:flex-row gap-3 min-h-0 overflow-hidden">
        <PlayerSidebar />

        {/* Center: Hangman game area */}
        <section className="flex-1 flex flex-col bg-gradient-to-b from-orange-50 to-white rounded-3xl shadow-[0_0_40px_-5px_rgba(0,0,0,0.6)] overflow-hidden relative border-4 border-orange-200 z-0 min-h-0 p-4 gap-3">
          ...
          <WordReveal
            ...
            currentWord={amDrawing ? wordDisplay : undefined}
            isWordSetter={amDrawing}
          />

          {/* Letter board (only for guessers) */}
          ...
          {/* Word setter overlay */}
          {amDrawing && (
            <div className="absolute top-3 right-3 bg-blue-500 text-white px-3 py-1.5 rounded-xl font-black text-xs border-2 border-blue-600 shadow-lg animate-bounce">
              <Icon name="edit_note" className="text-sm mr-1" />
              You set the word!
            </div>
          )}
        </section>
